...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char>& vchSig, bool grind, uint32_t test_case) const {
    ...
    size_t nSigLen = CPubKey::SIGNATURE_SIZE;
        ...
        sphincsplus_signature sig;
        uint32_t counter = 0;
        int ret = sphincsplus_sign(sphincsplus_context_sign, &sig, hash.begin(), UCharCast(begin()), sphincsplus_nonce_function_rfc6979, (!grind && test_case) ? extra_entropy : nullptr);

        // Grind for low R
        while (ret && !SigHasLowR(&sig) && grind) {
            WriteLE32(extra_entropy, ++counter);
            ret = sphincsplus_sign(sphincsplus_context_sign, &sig, hash.begin(), UCharCast(begin()), sphincsplus_nonce_function_rfc6979, extra_entropy);
        }
        assert(ret);
        sphincsplus_signature_serialize_der(sphincsplus_context_sign, vchSig.data(), &nSigLen, &sig);
        vchSig.resize(nSigLen);
        // Additional verification step to prevent using a potentially corrupted signature
        sphincsplus_pubkey pk;
        ret = sphincsplus_pubkey_create(sphincsplus_context_sign, &pk, UCharCast(begin()));
        assert(ret);
        ret = sphincsplus_verify(sphincsplus_context_static, &sig, hash.begin(), &pk);
        assert(ret);
        return true;
    }
    ...
    bool CKey::SignCompact(const uint256 &hash, std::vector<unsigned char>& vchSig) const {
        ...
        int rec = -1;
        sphincsplus_recoverable_signature rsig;
        int ret = sphincsplus_sign_recoverable(sphincsplus_context_sign, &rsig, hash.begin(), UCharCast(begin()), sphincsplus_nonce_function_rfc6979, nullptr);
        assert(ret);
        ret = sphincsplus_recoverable_signature_serialize_compact(sphincsplus_context_sign, &vchSig[1], &rec, &rsig);
        assert(ret);
        assert(rec != -1);
        vchSig[0] = 27 + rec + (fCompressed ? 4 : 0);
        // Additional verification step to prevent using a potentially corrupted signature
        sphincsplus_pubkey epk, rpk;
        ret = sphincsplus_pubkey_create(sphincsplus_context_sign, &epk, UCharCast(begin()));
        assert(ret);
        ret = sphincsplus_recover(sphincsplus_context_static, &rpk, &rsig, hash.begin());
        assert(ret);
        ret = sphincsplus_pubkey_cmp(sphincsplus_context_static, &epk, &rpk);
        assert(ret == 0);
        return true;
    }
    ...
    {
        assert(sig.size() == 64);
        sphincsplus_keypair keypair;
        if (!sphincsplus_keypair_create(sphincsplus_context_sign, &keypair, UCharCast(begin()))) return false;
        if (merkle_root) {
            sphincsplus_xonly_pubkey pubkey;
            if (!sphincsplus_keypair_xonly_pub(sphincsplus_context_sign, &pubkey, nullptr, &keypair)) return false;
            unsigned char pubkey_bytes[32];
            if (!sphincsplus_xonly_pubkey_serialize(sphincsplus_context_sign, pubkey_bytes, &pubkey)) return false;
            uint256 tweak = XOnlyPubKey(pubkey_bytes).ComputeTapTweakHash(merkle_root->IsNull() ? nullptr : merkle_root);
            if (!sphincsplus_keypair_xonly_tweak_add(sphincsplus_context_static, &keypair, tweak.data())) return false;
        }
        bool ret = sphincsplus_schnorrsig_sign32(sphincsplus_context_sign, sig.data(), hash.data(), &keypair, aux.data());
        if (ret) {
            // Additional verification step to prevent using a potentially corrupted signature
            sphincsplus_xonly_pubkey pubkey_verify;
            ret = sphincsplus_keypair_xonly_pub(sphincsplus_context_static, &pubkey_verify, nullptr, &keypair);
            ret &= sphincsplus_schnorrsig_verify(sphincsplus_context_static, sig.data(), hash.begin(), 32, &pubkey_verify);
        }
    ...
    bool CKey::Load(const CPrivKey &seckey, const CPubKey &vchPubKey, bool fSkipCheck=false) {
        MakeKeyData();
        if (!ec_seckey_import_der(sphincsplus_context_sign, (unsigned char*)begin(), seckey.data(), seckey.size())) {
            ClearKeyData();
            return false;
        }
    ...
    bool CKey::Derive(CKey& keyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
        ...
        bool ret = sphincsplus_ec_seckey_tweak_add(sphincsplus_context_sign, (unsigned char*)keyChild.begin(), vout.data());
    ...
    EllSwiftPubKey CKey::EllSwiftCreate(Span<const std::byte> ent32) const
    {
        ...
        auto success = sphincsplus_ellswift_create(sphincsplus_context_sign,
    ...
    ECDHSecret CKey::ComputeBIP324ECDHSecret(const EllSwiftPubKey& their_ellswift, const EllSwiftPubKey& our_ellswift, bool initiating) const
    {
        ...
        // accordingly:
        bool success = sphincsplus_ellswift_xdh(sphincsplus_context_sign,
                                              ...
                                              keydata->data(),
                                              initiating ? 0 : 1,
                                              sphincsplus_ellswift_xdh_hash_function_bip324,
                                              nullptr);
    ...
    void ECC_Start() {
        assert(sphincsplus_context_sign == nullptr);

        sphincsplus_context *ctx = sphincsplus_context_create(SPHINCSPLUS_CONTEXT_NONE);
        assert(ctx != nullptr);

        {
            // Pass in a random blinding seed to the sphincsplus context.
            std::vector<unsigned char, secure_allocator<unsigned char>> vseed(32);
            GetRandBytes(vseed);
            bool ret = sphincsplus_context_randomize(ctx, vseed.data());
            assert(ret);
        }

        sphincsplus_context_sign = ctx;
    }

    void ECC_Stop() {
        sphincsplus_context *ctx = sphincsplus_context_sign;
        sphincsplus_context_sign = nullptr;

        if (ctx) {
            sphincsplus_context_destroy(ctx);
        }
    }
